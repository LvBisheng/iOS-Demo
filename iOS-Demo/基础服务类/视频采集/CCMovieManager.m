...
- (instancetype)init {
    self = [super init];
    if (self) {
        _movieWritingQueue = dispatch_queue_create("com.lbs.test.video.writer", DISPATCH_QUEUE_SERIAL);
...
// 视频源数据写入配置
- (NSError *)setupAssetWriteVideoInput:(CMFormatDescriptionRef)currentFormatDescription {
...
- (CGAffineTransform)transformFromCurrentVideoOrientationToOrientation:(AVCaptureVideoOrientation)orientation {
    ...
    if(self.currentDevice.position == AVCaptureDevicePositionBack) {
        angleOffset = videoOrientationAngleOffset - orientationAngleOfffset + M_PI_2;
    } else {
        angleOffset = orientationAngleOfffset - videoOrientationAngleOffset + M_PI_2;
    }
...
// 获取视频旋转角度
-(CGFloat)angleOffsetFromPortraiOrientationToOrientation:(AVCaptureVideoOrientation)orientation {
    CGFloat angle = 0.0;
    switch (orientation) {
